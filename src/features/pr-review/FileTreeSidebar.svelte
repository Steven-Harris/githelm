...
<div class="w-64 bg-gray-50 border-r border-gray-200 h-full overflow-y-auto">
  <div class="p-4 border-b border-gray-200">
    <h3 class="text-sm font-medium text-gray-900">Files Changed ({files.length})</h3>
  </div>
  ...
  <div class="p-2">
    {#each files as file}
      <button
        ...
        class="flex items-center w-full py-2 px-2 text-sm rounded hover:bg-gray-100 {selectedFile === file.filename ? 'bg-blue-100 text-blue-800' : 'text-gray-700'}"
      >
        ...
        <div class="flex flex-col items-start min-w-0 flex-1">
          ...
          {#if file.filename.includes('/')}
            <span class="text-xs text-gray-500 truncate w-full">{file.filename.substring(0, file.filename.lastIndexOf('/'))}</span>
          {/if}
        </div>
