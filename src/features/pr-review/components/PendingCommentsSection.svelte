...
{#if selectedLines.length > 0 || pendingComments.length > 0}
  <div class="mt-3">
    ...
    {#if pendingComments.filter((c) => c.isPartOfReview && c.id !== activeCommentId).length > 0}
      ...
      <div class="mb-3">
        <h4 class="text-xs font-medium text-[#3fd382] uppercase tracking-wide mb-2">
          Pending Review Comments ({reviewComments.length})
        </h4>
        {#each reviewComments as comment}
          <div class="bg-[#121826] border border-[rgba(63,211,130,0.50)] rounded-lg p-2 mb-2">
            <div class="text-xs text-[#3fd382] font-medium mb-1">
              {getFileName(comment.filename)}: Line {comment.startLine} ({comment.side === 'left' ? 'original' : 'modified'})
            </div>
            <div class="text-sm text-[#e9eefb]">
              {comment.body}
            </div>
            <button onclick={() => onCancelComment && onCancelComment(comment.id)} class="text-xs text-[#9dabc4] hover:text-[#ffb3ae] mt-1"> Remove </button>
          </div>
        {/each}
      </div>
    {/if}
    ...
    {#if selectedLines.length > 0}
      ...
      <div class="mb-3">
        <h4 class="text-xs font-medium text-[#79b8ff] uppercase tracking-wide mb-2">Selected Lines</h4>

        {#if activeCommentId}
          {@const activeComment = pendingComments.find((c) => c.id === activeCommentId)}
          {#if activeComment}
            <!-- Unified card: location + textarea + actions -->
            <div bind:this={activeEditorEl} class="bg-[#121826] border border-[#2f6fd4]/40 rounded-lg p-3">
              <div class="font-mono text-xs text-[#e9eefb] mb-2">
              ...
              <div class="flex items-center justify-end mt-2">
                <div class="flex space-x-2">
                  <button
                    ...
                    class="px-3 py-1 text-xs text-[#9dabc4] hover:text-[#e9eefb] transition-colors"
                  >
                    Cancel
                  </button>
                  <button
          ...
          {:else}
            <div class="bg-[#121826] border border-[#243044] rounded-lg p-3">
              <div class="font-mono text-xs text-[#e9eefb]">
                {getFileName(selectedLines[0].filename)}
              </div>
              <div class="text-xs text-[#9dabc4] mt-1">Preparing comment editor…</div>
            </div>
          {/if}
        {/if}
      </div>
